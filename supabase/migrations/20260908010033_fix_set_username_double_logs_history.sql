-- BUG FIX: set_username() explicitly inserts into username_history AND the
-- trg_profiles_log_username trigger (AFTER UPDATE OF username -> log_username_history())
-- ALSO inserts into username_history for the same change. Every username set
-- (including the very first one at signup) logs TWO rows instead of one,
-- which immediately exhausts the 2-per-30-day quota (cfg_username_changes_per_30d)
-- for every new user before they've made a single real change. Fix: remove the
...
-- Found live-testing Epic A (QA-A08/A09/A39) on stance-capture-dev, 2026-09-08.
