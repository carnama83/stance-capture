...
BEGIN
  ...
  -- Fix (Sep 2026): the previous version (a) tried to cast the request tuple
  -- itself to json before calling extensions.http() at all, (b) read
  -- nonexistent current_setting() GUCs instead of vault secrets, and (c) had
  -- content/content_type swapped. Rewritten to match the proven-working
  -- pattern used by run_ingest_http()/run_cluster_http().
