...
//   • v11.1 (reconciled Sep 2026): pipeline_jobs logging added (job_type='question_draft').
//
...
async function startPipelineJob(sb, jobType, topicDraftId) {
  try {
    const { data, error } = await sb.from("pipeline_jobs").insert({
      job_type: jobType,
      status: "running",
      metadata: { topic_draft_id: topicDraftId ?? null }
    }).select("id").single();
    if (error) {
      console.warn("pipeline_job_start_failed_nonblocking:", error.message);
      return null;
    }
    return data?.id ?? null;
  } catch (e) {
    console.warn("pipeline_job_start_failed_nonblocking:", e?.message ?? String(e));
    return null;
  }
}
async function finishPipelineJob(sb, jobId, patch) {
  ...
  try {
    ...
    console.warn("pipeline_job_finish_failed_nonblocking:", e?.message ?? String(e));
  }
}
...
OUTPUT — return ONLY valid JSON, no markdown, no backticks:
{
  ...
  "share_headline": "A short teaser under 70 characters for link-preview cards, in DIFFERENT wording than the question — never a truncation or restatement of it. Compress the core event or tension into a fresh phrase. Example: if the question is 'Should Congress shield fossil fuel companies from climate liability lawsuits over decades of emissions?', a good share_headline is 'Fossil fuel companies face a legal reckoning' — same topic, distinct sentence.",
...
function parseQfResult(raw) {
  try {
    ...
    return {
      ...
      share_headline: parsed.share_headline?.trim() || null,
...
serve(async (req)=>{
  ...
  try {
    ...
    supabaseAdminForLog = supabaseAdmin;
    ...
    // 2b) Idempotency guard
    {
      ...
      if (exErr) {
        ...
        if (delErr) {
          ...
          await finishPipelineJob(supabaseAdmin, pipelineJobId, { status: "failed", finished_at: new Date().toISOString(), error_message: `Failed to replace rejected draft: ${delErr.message}`.slice(0, 800) });
        ...
        await finishPipelineJob(supabaseAdmin, pipelineJobId, { status: "success", finished_at: new Date().toISOString(), items_processed: 0, error_message: "existing_draft_returned_idempotent" });
    ...
    if (tdError) {
      await finishPipelineJob(supabaseAdmin, pipelineJobId, { status: "failed", finished_at: new Date().toISOString(), error_message: (tdError?.message ?? String(tdError)).slice(0, 800) });
      return jsonResponse({ error: "Failed to load topic draft", details: tdError?.message ?? String(tdError), code: tdError?.code ?? null }, 500);
    }
    if (!topicDraft) {
      await finishPipelineJob(supabaseAdmin, pipelineJobId, { status: "failed", finished_at: new Date().toISOString(), error_message: "Topic draft not found" });
      return jsonResponse({ error: "Topic draft not found" }, 404);
    }
    ...
    if (!apiKey) {
      ...
      try {
        ...
        if (!result) {
          ...
          shareHeadline = result.share_headline;
    ...
    if (draftError || !draft) {
      ...
      await finishPipelineJob(supabaseAdmin, pipelineJobId, { status: "failed", finished_at: new Date().toISOString(), error_message: (draftError?.message ?? String(draftError)).slice(0, 800) });
    ...
    await finishPipelineJob(supabaseAdmin, pipelineJobId, {
      status: "success",
    ...
    if (pipelineJobId && supabaseAdminForLog) {
      await finishPipelineJob(supabaseAdminForLog, pipelineJobId, {
        status: "failed",
        finished_at: new Date().toISOString(),
