...
// v2.1 — pipeline_jobs logging added (reconciled Sep 2026).
// v2.2 — removed CLASSIFY_WINDOW_HOURS recency filter on the unclassified-drafts
// query (reconciled Sep 2026): it caused any draft that failed to get a parent
// (low confidence, or ran before its target theme existed) to permanently age
// out of consideration after 72h with no retry path. Now loads ALL drafts with
// parent_topic_id IS NULL, oldest first, up to CLASSIFY_BATCH per run.
//
...
async function startPipelineJob(sb, jobType, traceId) {
  try {
...
async function finishPipelineJob(sb, jobId, patch, traceId) {
  if (!jobId) return;
  try {
...
Deno.serve(async (req)=>{
  ...
  const pipelineJobId = await startPipelineJob(sb, "classify_parent_topics", traceId);
  // ── Step 1: Load unclassified topic_drafts ────────────────────────────────
  // No age filter: a draft with parent_topic_id IS NULL is unclassified regardless of
  // when it was created. Oldest-first so a backlog of new drafts can't starve older
  // stuck ones out of every future BATCH-sized run indefinitely.
  const { data: drafts, error: draftsErr } = await sb.from("topic_drafts").select("id, title, summary, tags, location_label").is("parent_topic_id", null).order("created_at", {
    ascending: true
  }).limit(BATCH);
  if (draftsErr) {
    ...
    await finishPipelineJob(sb, pipelineJobId, {
      status: "failed",
      finished_at: new Date().toISOString(),
      error_message: draftsErr.message.slice(0, 800)
    }, traceId);
    return new Response(JSON.stringify({
      ok: false,
      error: draftsErr.message
  ...
  await finishPipelineJob(sb, pipelineJobId, {
    status: "success",
    finished_at: new Date().toISOString(),
    items_processed: classified + proposed,
    error_message: errors.length ? errors.slice(0, 5).join(" | ").slice(0, 800) : null
  }, traceId);
  return new Response(JSON.stringify({
    ok: true,
    traceId,
