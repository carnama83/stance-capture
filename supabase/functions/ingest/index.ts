...
Deno.serve(async (req)=>{
  ...
  const pipelineJobId = await startPipelineJob("ingest", traceId);
  try {
    ...
    await finishPipelineJob(pipelineJobId, {
      ...
      duration_ms: done.duration_ms,
      items_processed: summary?.processed ?? summary?.inserted ?? 0
    }, traceId);
    ...
    await finishPipelineJob(pipelineJobId, {
      ...
      finished_at: new Date().toISOString(),
      duration_ms: done.duration_ms,
      error_message: (err?.message ?? String(err))?.slice(0, 800)
    }, traceId);
    log("error", "exception", {
      ...done,
