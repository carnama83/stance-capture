...
//
// v6.1 — pipeline_jobs logging added (reconciled Sep 2026, job_type='create_topic_drafts').
//
...
Deno.serve(async (req)=>{
  ...
  const pipelineJobId = await startPipelineJob(sb, "create_topic_drafts", traceId);
  const errors = [];
  let draftsCreated = 0;
  let skipped = 0;
  try {
    ...
    if (clErr) {
      ...
      await finishPipelineJob(sb, pipelineJobId, { status: "failed", finished_at: new Date().toISOString(), error_message: clErr.message.slice(0, 800) }, traceId);
      return new Response(JSON.stringify({
        ok: false,
        error: clErr.message
      }), {
    ...
    if (clusters.length === 0) {
      ...
      await finishPipelineJob(sb, pipelineJobId, { status: "success", finished_at: new Date().toISOString(), items_processed: 0 }, traceId);
      return new Response(JSON.stringify({
        ok: true,
        traceId,
        draftsCreated: 0,
    ...
    await finishPipelineJob(sb, pipelineJobId, { status: "failed", finished_at: new Date().toISOString(), error_message: String(err?.message ?? err).slice(0, 800) }, traceId);
    return new Response(JSON.stringify({
      ok: false,
      traceId,
      error: err?.message
