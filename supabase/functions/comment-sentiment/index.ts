...
//
// G-04 fix (Sep 2026): the OpenAI call passed `response_format` to the *Responses* API.
// That parameter belongs to Chat Completions; in the Responses API it moved to
// `text.format`. OpenAI rejected every request with:
//   400 Unsupported parameter: 'response_format'. ... this parameter has moved to 'text.format'.
// The broad catch below swallowed that 400 and fell through to the score=0 / "neutral"
// default, so EVERY comment was silently stored as sentiment_score 0 / label 'neutral'
// while the endpoint kept returning HTTP 200. Fixed to `text: { format: ... }`.
//
// The failure was invisible because success and AI-failure both reported the same
// `source` value, so `source` now distinguishes them: "ai" vs "neutral_fallback_error".
// Behaviour is unchanged - still fail-open, never 500 the browser.
...
`;
      const result = await client.responses.create({
        model: "gpt-4o-mini",
        input: prompt,
        // G-04: Responses API uses text.format, NOT response_format.
        text: {
          format: {
            type: "json_object"
          }
    ...
    return await updateAndReturn(score, label, aiOk ? "ai" : "neutral_fallback_error");
  } catch (err) {
