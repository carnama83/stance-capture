...
//
// v3.8 — Step 4 candidate query moved to SQL (get_unclustered_candidates RPC):
//   Previously this step pulled the top CANDIDATE_LIMIT rows from ingestion_queue
//   ordered by created_at DESC within the window, regardless of clustered status,
//   then filtered out already-clustered rows afterward in Step 7. That meant
//   already-clustered rows could occupy slots ahead of the LIMIT cutoff, pushing
//   genuinely unclustered rows — especially older ones still inside the window —
//   out of the candidate set entirely on every run. Confirmed against real data:
//   522 rows in the 72h window, 392 unclustered, but only 145 of those 392 fell
//   inside the old top-200 slice; 247 were starved out every run until they aged
//   past the window. The new get_unclustered_candidates() Postgres function does
//   the NOT EXISTS-against-topic_cluster_items exclusion in SQL, before LIMIT is
//   applied, so the candidate slots are spent only on rows that actually need
//   clustering. This does NOT weaken duplicate-cluster protection — that lives
//   entirely in Step 13c's cross-run centroid merge against topic_clusters, which
//   is unaffected by what Step 4 selects. Step 7's idempotency check is left in
//   place below as a defensive no-op rather than removed.
//
// v3.9 — Cross-run merge cleanup (M-B28, found live in UAT QA, Sep 2026):
//   Steps 10/11 unconditionally persist every newly-seeded cluster and its items
//   BEFORE Step 13c's cross-run merge check runs. Previously, when a new cluster
//   turned out to match an existing one closely enough to merge (>= MERGE_THRESHOLD),
//   13c copied its items into the existing cluster and updated that cluster's
//   centroid — but never removed the now-redundant new cluster row or its
//   now-duplicate topic_cluster_items rows. That left two topic_clusters rows for
//   the same real-world story, with items double-counted across both, indefinitely.
//   Fix: Step 13c-iii now deletes the merged-away cluster's topic_cluster_items and
//   topic_clusters rows once its items have been copied into the existing cluster.
//   The `updated`/`clusters` counters returned to the caller now also reflect this
//   (updated = clusters actually merged, clusters = net NEW clusters left standing).
//
...
export async function run(ctx) {
  ...
  // Filtering to unclustered rows now happens in SQL (get_unclustered_candidates),
  // before CANDIDATE_LIMIT is applied — previously this query pulled the top
  // CANDIDATE_LIMIT rows by created_at DESC regardless of clustered status, so
  // already-clustered rows could occupy slots and push still-unclustered rows
  // (esp. older ones within the window) out of the candidate set entirely, run
  // after run, until they aged past the window. Step 7 below still re-checks
  // idempotency as a cheap defensive no-op — kept intentionally rather than
  // removed, in case anything upstream ever changes.
  log("info", "📋 STEP 4: Loading candidates WITH embeddings", {});
  const { data: embRows, error: embErr } = await sb.rpc("get_unclustered_candidates", {
    p_since: sinceIso,
  ...
  // 13c-iii: Clean up the now-redundant new cluster row(s) whose items were just
  // merged into an existing cluster above. Steps 10/11 unconditionally persist
  // every newly-seeded cluster and its items BEFORE this merge check runs, so
  // without this cleanup a merged-away cluster's topic_clusters row — and its
  // now-duplicate topic_cluster_items rows under the new cluster_id — were left
  // behind indefinitely, double-counting items across two clusters for the same
  // story (found live in UAT QA, M-B28).
  const mergedAwayClusterIds = [...newClusterMergeMap.keys()];
  if (mergedAwayClusterIds.length > 0) {
    const { error: delItemsErr } = await sb.from("topic_cluster_items").delete().in("cluster_id", mergedAwayClusterIds);
    if (delItemsErr) log("warn", "⚠️ cleanup: delete merged-away cluster_items failed (non-fatal)", {
      error: delItemsErr.message,
      clusterIds: mergedAwayClusterIds
    });
    const { error: delClustersErr } = await sb.from("topic_clusters").delete().in("id", mergedAwayClusterIds);
    if (delClustersErr) log("warn", "⚠️ cleanup: delete merged-away topic_clusters failed (non-fatal)", {
      error: delClustersErr.message,
      clusterIds: mergedAwayClusterIds
    });
    log("info", "🧹 Cleaned up merged-away new cluster rows", {
      clustersRemoved: mergedAwayClusterIds.length
    });
  }
  ...
  return {
    clusters: valid.length - newClusterMergeMap.size,
    items: createdItems,
    updated: newClusterMergeMap.size,
    draftsCreated,
    skipped,
    failed,
    errors: errors.length ? errors : undefined,
