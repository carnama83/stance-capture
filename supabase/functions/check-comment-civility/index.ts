...
//
// G-05 fix (Sep 2026): the auto-moderation notification linked to `/questions/<id>`,
// which is not a route in this app. App.tsx defines only `/q/:id`, so the link fell
// through to the catch-all NotFound — every "Your comment was removed" notification
// was a dead link. All other notification rows already use `/q/<id>`; this producer
// was the sole outlier. Changed to `/q/<id>` to match.
  ...
  try {
    ...
    if (commentId) {
      ...
      if (flagged && commentUserId) {
        ...
        // G-05: the question detail route is /q/:id — /questions/:id does not exist.
        const notifHref = questionId ? `/q/${questionId}` : "/settings/account";
        // Notify the user
