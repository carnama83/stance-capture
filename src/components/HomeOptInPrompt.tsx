  ...
  if (!open) {
    return (
      <div className="fixed bottom-4 inset-x-0 z-50 flex justify-center px-4 pointer-events-none">
        ...
        >
          {t("homeOptInPrompt.pillCta", { count: stagedCount })}
        </button>
      </div>
    );
  }
      ...
      <div className="pointer-events-auto w-full max-w-sm rounded-2xl border border-violet-200 bg-white p-4 shadow-2xl space-y-3">
        <div className="flex items-start justify-between">
          <div>
            <p className="text-sm font-semibold text-slate-800">
              {t("homeOptInPrompt.recordedCount", { count: stagedCount })}
            </p>
            <p className="text-xs text-slate-500">{t("homeOptInPrompt.notCountedYet", { count: stagedCount })}</p>
          </div>
        ...
        {emailSent ? (
          <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-800">
            {t("homeOptInPrompt.checkYourEmailForA")}
          </div>
        ) : (
        ...
        <div className="flex items-center gap-3">
          <div className="h-px flex-1 bg-slate-200" /><span className="text-xs text-slate-400">{t("webOptIn.or")}</span><div className="h-px flex-1 bg-slate-200" />
        </div>
