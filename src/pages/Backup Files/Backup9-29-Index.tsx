...
// --------------------- Data adapters ---------------------
// Try several tables/views; return the first that succeeds with rows
async function trySelectTopics(sb: any, options: {
  ...
  for (const source of sourceCandidates) {
    try {
      let q = sb.from(source).select(select).limit(limit);

      // best-effort search (if "title" exists; if it doesn't, we'll just ignore the error)
      if (search && search.trim()) {
        // Prefer ilike on title; if it fails, we'll filter client-side later
        q = q.ilike?.("title", `%${search.trim()}%`) ?? q;
      }

      // Try ordering by likely columns in priority order
      ...
      for (let i = 0; i <= orderCandidates.length; i++) {
        ...
        if (!error) {
          // If we searched but ilike is unsupported: do a client-side filter as fallback
          ...
          // If no rows, still fall through to try next source/candidate
          dataResp = rows;
          break;
        }
      }
...
async function fetchTrendingTopics(sb: any, opts: { personalized: boolean; userId?: string | null }) {
  // If you later add user-personalized views, read opts.userId to conditionally query.
...
async function fetchTopicsGrid(sb: any, opts: { search: string; page: number; pageSize: number }) {
  ...
  // Try specialized sources first; fall back to `topics`.
  const rows = await trySelectTopics(sb, {
    ...
    limit: pageSize, // we'll fetch page by page
    orderCandidates: ["activity_7d", "updated_at"],
    search,
  });

  if (rows.length > 0) {
    // emulate pagination client-side if the source doesn't support range()
    return {
...
export default function Index() {
  ...
  // return-to logic for anonymous action
