// src/pages/Index.tsx — clean merged version (preserves existing functionality)
  ...
  for (const sourceLabel of sourceCandidates) {
    ...
    try {
      ...
      for (let i = 0; i <= orderCandidates.length; i++) {
        ...
        const run = orderBy ? (q as any).order(orderBy as any, { ascending: false }) : q;
...
async function fetchTrendingTopics(sb: any, _opts: { personalized: boolean; userId?: string | null }) {
  const rows = await trySelectTopics(sb, {
    sourceCandidates: TRENDING_SOURCES,
    defaultSelect: "id,title,summary,tags,updated_at,tier,location_label,trending_score,activity_7d",
...
function TopicGrid(props: {
  ...
  const { interactive, onRequireLogin, search, setSearch, page, setPage, pageSize, loading, result } = props;
        ...
        <>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {items.map((t) => (
              <div key={t.id} className="rounded-lg border p-4">
                ...
                {t.location_label && <div className="text-[11px] text-slate-500 mt-0.5">{t.location_label}</div>}
          ...
          <div className="flex items-center justify-between mt-4">
            ...
            <div className="flex gap-2">
              <button className="rounded border px-3 py-1.5 text-sm" onClick={() => setPage(Math.max(0, page - 1))} disabled={page === 0}>
                Prev
              </button>
              <button className="rounded border px-3 py-1.5 text-sm" onClick={() => setPage(page + 1)} disabled={!result?.hasMore}>
                Next
              </button>
            </div>
          </div>
