...
function LocationSelect(props: {
  ...
  React.useEffect(() => {
    (async () => {
      ...
      if (!error && data) {
        ...
        console.warn("[countries] error:", error.message);
      }
    })();
  }, []);
  ...
  React.useEffect(() => {
    (async () => {
      ...
      if (!error && data) {
        setStates(
          data.map((row: any) => ({
            code: (row.iso_code as string).split("-")[1],
            name: row.name as string,
          }))
        );
      } else if (error) {
  ...
  React.useEffect(() => {
    (async () => {
      ...
      if (!error && data) {
        setCounties(
          data.map((row: any) => ({
            code: row.code as string, // full iso_code of county
            name: row.name as string,
          }))
        );
      } else if (error) {
        console.warn("[counties] error:", error.message);
      }
    })();
  }, [props.stateCode]);
  ...
  React.useEffect(() => {
    (async () => {
      ...
      // Use locations table: city iso_code starts with county iso_code + "-"
      const { data, error } = await sb
        .from("locations")
        .select("iso_code, name")
        .eq("type", "city")
      ...
      if (!error && data) {
        ...
        console.warn("[cities] error:", error.message);
      }
    })();
  }, [props.countyCode]);
...
export default function SignupPage() {
  ...
  function validate(): SignupErrors {
    ...
    } else if (gender === "self_described" && !genderSelf.trim()) {
  ...
  async function resolveLocation(): Promise<{ locationId: string; precision: Precision } | null> {
    // City
    if (cityCode) {
      const r = await sb
        .from("locations")
      ...
      if (!r.error && r.data?.id) {
        return { locationId: r.data.id, precision: "city" };
      }
    }
  ...
  async function finalizeOnboarding() {
    ...
    // Gender (with fallback to direct update)
