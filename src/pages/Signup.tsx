    ...
    async (country_code: string) => {
      ...
      try {
        ...
        if (r.error) {
          ...
          console.info(`[states] ${country_code} -> ${(r.data ?? []).length} row(s)`);
          setStates(r.data ?? []);
        }
      } catch (e) {
    ...
    async (state_code: string) => {
      ...
      try {
        ...
        if (r.error) {
          ...
          console.info(`[counties] ${state_code} -> ${(r.data ?? []).length} row(s)`);
          setCounties(r.data ?? []);
        }
      } catch (e) {
    ...
    async (state_code: string, county_code?: string) => {
      ...
      try {
        ...
        if (r.error) {
          ...
          console.info(
            `[cities] ${state_code}${county_code ? " / " + county_code : ""} -> ${(r.data ?? []).length} row(s)`
          );
          setCities(r.data ?? []);
        }
      } catch (e) {
    ...
    <div className="space-y-2" aria-live="polite">
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        ...
        <div>
          ...
          <select
            id="country"
            className={cx("mt-1 w-full rounded-lg border p-2", props.errorCountry && "border-rose-500")}
            value={props.country}
          ...
          {props.errorCountry && <p className="mt-1 text-xs text-rose-600">{props.errorCountry}</p>}
        </div>
        ...
        <div>
          ...
          <select
            id="state"
            className={cx("mt-1 w-full rounded-lg border p-2", props.errorState && "border-rose-500")}
          ...
          >
            ...
            {props.country && loadingStates && <option value="">Loading states…</option>}
          ...
          {props.country && !loadingStates && states.length === 0 && (
            <p className="mt-1 text-xs text-amber-600">No states found for this country.</p>
          )}
          {props.errorState && <p className="mt-1 text-xs text-rose-600">{props.errorState}</p>}
        </div>
        ...
        <div>
          ...
          <select
            id="county"
            className={cx("mt-1 w-full rounded-lg border p-2", props.errorCounty && "border-rose-500")}
          ...
          >
            ...
            {props.stateCode && loadingCounties && <option value="">Loading counties…</option>}
          ...
          {props.errorCounty && <p className="mt-1 text-xs text-rose-600">{props.errorCounty}</p>}
        </div>
        ...
        <div>
          ...
          <input
            id="city"
            list="city-options"
            className={cx("mt-1 w-full rounded-lg border p-2", props.errorCity && "border-rose-500")}
            placeholder={!props.stateCode ? "Select state first" : loadingCities ? "Loading cities…" : "Type to search…"}
            value={cityText}
            onChange={(e) => {
            ...
            onBlur={() => {
              const match = cities.find((c) => c.name.toLowerCase() === cityText.toLowerCase());
              if (!match) {
                props.setCityId("");
                setCityText("");
              }
          ...
          {props.errorCity && <p className="mt-1 text-xs text-rose-600">{props.errorCity}</p>}
        </div>
      </div>
    </div>
  );
...
export default function Signup() {
  ...
  function focusFirstError(errs: Record<string, string>) {
    ...
    for (const key of order) {
      ...
      if (key === "email") el = refEmail.current;
        ...
        el = refDobContainer.current?.querySelector("input,select,textarea") as HTMLElement | null;
  ...
  > {
    ...
    if (countyCode) {
      const r = await sb.from("locations").select("id").eq("type", "county").eq("iso_code", countyCode).limit(1).single();
      if (!r.error && r.data?.id) return { locationId: r.data.id, precision: "county" };
    }

    if (stateCode) {
      const guesses = country ? [stateCode, `${country}-${stateCode}`] : [stateCode];
      const r = await sb.from("locations").select("id, iso_code").eq("type", "state").in("iso_code", guesses);
      const row = r.data?.[0];
      if (!r.error && row?.id) return { locationId: row.id, precision: "state" };
    }

    if (country) {
      const r = await sb.from("locations").select("id").eq("type", "country").eq("iso_code", country).limit(1).single();
      if (!r.error && r.data?.id) return { locationId: r.data.id, precision: "country" };
    }
  ...
  async function finalizeOnboarding() {
    ...
    if (username.trim()) {
      const u = await sb.rpc("set_username", { p_username: username.trim().toLowerCase() });
    ...
    if (hasAny) {
      ...
      if (resolved) {
        ...
        if (who.error || !who.data) throw who.error || new Error("whoami() failed");
  ...
  async function onSignup(e: React.FormEvent) {
    ...
    try {
      ...
      stashForFirstLogin();
      setMsg("Check your email to confirm your account. After you log in, we’ll finish setting up your profile automatically.");
    } catch (err: any) {
        ...
        <form className="space-y-4" onSubmit={onSignup} noValidate>
          ...
          <div>
            ...
            <input
              ...
              ref={refEmail}
              type="email"
              className={cx("mt-1 w-full rounded border px-3 py-2", errors.email && "border-rose-500")}
          ...
          <div>
            ...
            <input
              ...
              ref={refPassword}
              type="password"
              className={cx("mt-1 w-full rounded border px-3 py-2", errors.password && "border-rose-500")}
          ...
          <div ref={refDobContainer}>
            ...
            <p className="mt-1 text-xs text-muted-foreground">You must be 13 or older. This is not shown publicly.</p>
          ...
          <fieldset>
            ...
            <div className="mt-2 grid grid-cols-2 gap-2 md:grid-cols-3">
              {(["male", "female", "nonbinary", "prefer_not_to_say", "self_described"] as Gender[]).map((g) => (
                <label key={g} className="inline-flex items-center gap-2 rounded-lg border p-2">
                  <input type="radio" name="gender" value={g} checked={gender === g} onChange={() => setGender(g)} />
                  <span className="text-sm capitalize">{g.replaceAll("_", " ")}</span>
                </label>
              ))}
            </div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">Used for aggregate analytics only; never shown with your identity.</p>
          </fieldset>
          ...
          {/* Submit */}
          <button type="submit" className="w-full rounded bg-slate-900 py-2 text-white disabled:opacity-60" disabled={busy}>
