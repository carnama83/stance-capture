...
export default function Signup() {
  ...
  // Resolve a locations.id and precision from the current selection
  async function resolveLocationForSelection(): Promise<
    | { locationId: string; precision: Precision }
    | null
  > {
    // City chosen (geo_cities_v already returns locations.id as "id")
    if (cityId) {
      return { locationId: cityId, precision: "city" };
    }

    // County by iso_code (geo_counties_v.code ~= locations.iso_code)
    if (countyCode) {
      const r = await sb
        .from("locations")
        .select("id")
    ...
    // State by iso_code: locations.iso_code may be "US-NJ" or just "NJ"
    if (stateCode) {
  ...
  async function finalizeOnboarding() {
    // Ensure profile exists (random_id etc.)
    ...
    // DOB via text overload
    if (dob) {
      const d = await sb.rpc("profile_set_dob_checked", { p_dob_text: dob });
      if (d.error) throw d.error;
    }

    // Gender via RPC (fallback to direct update if needed)
